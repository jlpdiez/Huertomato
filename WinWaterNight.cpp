...
WinWaterNight::WinWaterNight(const WinWaterNight &other) : Window(other) {
	for (uint8_t i = 0; i < _nWaterNightButtons; i++) {
...
void WinWaterNight::print() {
	...
	_waterNightButtons[_nFlowButtons] = _buttons.addButton(_xConfig+2*_bigFontSize,_yThreeLnsFirst,waterNightStr0);
	//Watering at night ON/OFF
	if (_nightWater)
		_lcd->print(pmChar(onStr),_xConfig+_bigFontSize*2+_bigFontSize*strlen_P(waterNightStr0),_yThreeLnsFirst);
	else
		_lcd->print(pmChar(offStr),_xConfig+_bigFontSize*2+_bigFontSize*strlen_P(waterNightStr0),_yThreeLnsFirst);
	
	
	//Second Line
	_lcd->setColor(grey[0],grey[1],grey[2]);
	int x = _xConfig;
	_lcd->print(pmChar(rawLight),x,_yThreeLnsSecond);
	x += _bigFontSize * (strlen_P(rawLight) + 1);
	_lcd->printNumI(_rawLightLvl,x,_yThreeLnsSecond,4);
	x +=5*_bigFontSize;
	_lcd->print(pmChar(newLightUnit),x,_yThreeLnsSecond);
	
	//Third Line
	x = _xConfig;
	_lcd->print(pmChar(lightThreshold),x,_yThreeLnsThird);
...
Window::Screen WinWaterNight::processTouch(const int x, int y) {
	...
	//Back
	if (buttonIndex == _waterNightButtons[0]) 
